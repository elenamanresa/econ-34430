...
G = array(0,c(nl,nk,nk)) # def: conditional distribution k' given k, l
...
#=wireframe(G[1,,],aspect = c(1,1),xlab = "previous firm",ylab="next firm")
...
```{r}
wireframe(H,aspect = c(1,1), xlab = "worker skills",ylab="job skills")
```
...
data[,alpha:=alpha*2+3]
```


## Calculating rank-rank regression

```{r}

lmr_model <- lmranks(r(alpha) ~ r(psi), data=data)
summary(lmr_model)

```

## Calculating rank-rank regression coarsening the heterogeneity
...
# let's draw some FE
psi2   = qnorm(1:nk2/(nk2+1)) * alpha_sd
alpha2 = qnorm(1:nl2/(nl2+1)) * psi_sd

data[,alpha2_pos:=0]
alpha_list = data$alpha
psi_list = data$psi
alpha_list2 = c(0,ni*nt)
psi_list2 = c(0,ni*nt)
pos_alpha=c(0,ni*nt)
pos_psi=c(0,ni*nt)
for(i in 1:ni){
  for(t in 1:nt){
    ind = (i-1)*nt+t
    pos_alpha[ind] = which.min(abs(alpha_list[ind]-alpha2))
    pos_psi[ind] = which.min(abs(psi_list[ind]-psi2))
    alpha_list2[ind] = alpha2[pos_alpha[ind]]
    psi_list2[ind] = psi2[pos_psi[ind]]
  }
}

data[,alpha2:=alpha_list2]
data[,psi2:=psi_list2]

lmr_model_coarse <- lmranks(r(alpha2) ~ r(psi2), data=data)
summary(lmr_model_coarse)
